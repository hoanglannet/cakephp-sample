<?php
class AjaxPaginatorHelper extends AppHelper {
 ...
 * @access public
 */
	function prev($title = '<< Previous', $ajaxFunction, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		return $this->__pagingLink('Prev', $ajaxFunction, $title, $options, $disabledTitle, $disabledOptions);
	}
 ...
 * @access public
 */
	function next($title = 'Next >>', $ajaxFunction, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		return $this->__pagingLink('Next', $ajaxFunction, $title, $options, $disabledTitle, $disabledOptions);
	}
 ...
 * @access protected
 */
	function __pagingLink($which, $ajaxFunction, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		$check = 'has' . $which;
		$_defaults = array(
		...
		$url = 'javascript:' . $ajaxFunction . "(" . ($paging['page'] + ($which == 'Prev' ? $step * -1 : $step)) . ")";
		//array_merge(array('page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step)), $url);
		//var_dump($url);
		if ($this->{$check}($model)) {
			return $this->Html->tag($tag, "<a href='" . $url . "'>" . $title . "</a>", compact('class'));
		} else {
 ...
 * @access public
 */
	function numbers($ajaxFunction, $options = array()) {
		if ($options === true) {
			$options = array(
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				$out .= $this->Html->tag($tag, "<a href='javascript:" . $ajaxFunction . '(' . $i . ')' . "'>" . $i . "</a>")
					. $separator;
			}
			...
			if ($end != $params['page']) {
				$out .= $this->Html->tag($tag, "<a href='javascript:" . $ajaxFunction . '(' . $end . ')' . "'>" . $i . "</a>");
			}
